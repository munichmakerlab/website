...
TumblrRSS.prototype.getPhotoMarkup = function(post) {
	...
	if (post.photos.length > 0) {
		...
		post.photos.forEach(function(photo){
			markup.push('<li>');
			markup.push('<a href="' + photo['photo-url-1280'] + '" data-lightbox="' + post['id'] + '">');
			markup.push('<img class="image" src="' + photo['photo-url-500'] + '">');
			markup.push('</a></li>');
		});
		markup.push('</ul>');
		markup.push('</div>');
	} else {
		markup.push('<div class="image_container">');
		markup.push('<a href="' + post['photo-url-1280'] + '" data-lightbox="' + post['id'] + '">');
		markup.push('<img class="image" src="' + post['photo-url-500'] + '" data-lightbox="' + post['id'] + '">');
		markup.push('</a></div>');
	}
